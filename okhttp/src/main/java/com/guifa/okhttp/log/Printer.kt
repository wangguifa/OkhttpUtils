...
class Printer private constructor() {
    ...
    companion object {
        ...
        fun printJsonRequest(
            ...
            getRequest(builder.level, header, method)?.let {
                logLines(
                    builder.type,
                    tag,
                    it,
            ...
            if (builder.level == Level.BASIC || builder.level == Level.BODY) {
                LINE_SEPARATOR?.let { requestBody.split(it).toTypedArray() }?.let {
                    logLines(
                        builder.type,
                        tag,
                        it,
        ...
        fun printJsonResponse(
            ...
            if (builder.level == Level.BASIC || builder.level == Level.BODY) {
                LINE_SEPARATOR?.let { responseBody.split(it).toTypedArray() }?.let {
                    logLines(
                        builder.type,
                        tag,
                        it,
        ...
        private fun getRequest(level: Level, headers: Headers, method: String): Array<String>? {
            ...
            log =
                METHOD_TAG + method + DOUBLE_SEPARATOR + when {
                    isEmpty("$headers") -> ""
            ...
            return LINE_SEPARATOR?.let { log.split(it).toTypedArray() }
        }
